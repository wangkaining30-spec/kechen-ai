// ============================================================================
//  ai_brain_v7.cpp — 迷你 Transformer (RNN + 2 层多头自注意力) 字符级中文语言模型
//
//  相对 v6 的两大升级:
//   1. 语料再扩充 (46 段 ~676 字符 -> 61 段 ~1033 字符):
//        保留 v6 全部语料: 古诗/谚语 + 迷你字典 + 对话问答 + 论语 + 新诗
//        新增: 10 首新诗 (春望/绝句/望天门山/黄鹤楼送孟浩然/赠汪伦/
//              九月九日忆山东兄弟/回乡偶书/凉州词/出塞/芙蓉楼送辛渐)
//        新增: 5 条新对话 (你会背诗吗=我会背很多古诗 / 你最喜欢哪首诗=静夜思 /
//              给我讲个笑话=程序员的幽默是0和1 / 你能做什么=背诗、聊天、认数字 /
//              你饿了吗=我不吃饭，我只吃数据)
//   2. 权重自动保存与加载 (秒启动):
//        训练完成后自动把全部模型权重保存到 ai_brain_v7_weights.bin 文件;
//        下次启动时若检测到权重文件 -> 直接加载, 跳过训练, 秒启动。
//  模型结构: (同 v6)
//     字符 Embedding (V x H)
...
//
//  编译: clang++ -O2 -std=c++17 ai_brain_v7.cpp -o ai_brain_v7
//  运行: ./ai_brain_v7          (首次: 训练 + 自动保存权重;
//                               再次: 检测到 ai_brain_v7_weights.bin 直接加载, 秒启动)
// ============================================================================
#include <algorithm>
#include <chrono>
#include <cmath>
...
public:
    ...
    static constexpr uint32_t W_MAGIC = 0x41564237;   // "AVB7"
...
int main() {
    ...
    // 内置中文语料 (v7 扩充版, 共 61 段):
    //   v5/v6 保留: 11 段古诗/谚语 + 10 条迷你中文字典 + 10 条对话问答
    //               + 10 条《论语》名句 + 5 首新诗
    //   v7 新增: 10 首新诗 + 5 条新对话
    const char* corpus[] = {
    ...
    // v7 模型结构同 v6: hidden 128, FFN 256, Transformer 块 2 层
    const int H = 128, NH = 4, FF = 256, T_MAX = 48, NBLK = 2;
    const string WFILE = "ai_brain_v7_weights.bin";   // v7 权重文件
    CharRNNTransformer model((int)tok.itos.size(), H, NH, FF, T_MAX, NBLK, rng);

    cout << "========== 迷你 Transformer (RNN + 2 层多头自注意力) 中文语言模型 v7 ==========\n";
    cout << "语料 : " << n_lines << " 段 (11 古诗/谚语 + 10 迷你字典 + 10 对话问答\n";
    cout << "      + 10《论语》名句 + 5 新诗 + 10 新诗 + 5 新对话), 共 " << total_chars << " 字符\n";
    ...
    bool loaded = false;
    {
        ...
        if (exists && model.load_weights(WFILE, tok)) {
            ...
            printf("[权重] 检测到 %s，已加载已训练好的模型 -> 跳过训练 (秒启动, 加载耗时 %lld ms)\n\n",
                   WFILE.c_str(), (long long)ms);
        } else if (exists) {
    ...
    if (!loaded) {
        ...
        printf("      + 10 条《论语》名句 + 5 首新诗 + 10 首新诗 + 5 条新对话 (共 61 段, %d 字符),\n", total_chars);
        printf("      全部共享同一词表与同一套参数记忆。\n");
        printf("      固有歧义位置 (上下文相同而下一字符不同, 无法完全记住):\n");
        printf("      ① <BOS> 后首字符 61 选 1 (你×10, 学×3, 床/明/千/知/天/少×2, 其余各 1);\n");
        printf("      ② <BOS>你 -> 好/叫/是/会/多/开/最/能/饿 十选一 (会×2);\n");
        printf("      ③ <BOS>床->前/=、<BOS>明->月/=、<BOS>千->里/山、<BOS>知->之/=、\n");
        printf("         <BOS>天->街/门、<BOS>少->壮/小;\n");
        printf("      ④ <BOS>学而->时/不、<BOS>学而不->思/厌;\n");
        printf("      理论最小平均损失 ≈ 0.22 (为下界);\n");
        printf("      其余位置 (含全部 15 条问答对的回答部分) 均可被模型完全记住。\n\n");

        // ==================== v7 核心升级 2: 训练完成后自动保存权重 ====================
        if (model.save_weights(WFILE, tok)) {
            printf("[权重] 训练完成，模型权重已自动保存到 %s\n", WFILE.c_str());
    ...
    printf("\n[自检] 逐位置核对 (应与语料完全一致):\n");
    {
        ...
        printf("  命中 %d/%d 位置 (未命中均为固有歧义: <BOS> 后首字符 61 选 1,\n",
               total - bad, total);
        printf("       以及 <BOS>床/明/千/知/天/少 后二选一、<BOS>你 后十选一、\n");
        printf("       <BOS>学而/学而不 后二选一等)\n");
    }

    // ---- 自动演示续写 (v7: 覆盖旧诗 + 新诗 + 论语) ----
    ...
    run_demo("两个黄鹂", "绝句    ");
    run_demo("天门中断", "望天门山");
    run_demo("故人西辞", "黄鹤楼送孟浩然");
    run_demo("李白乘舟", "赠汪伦  ");
    run_demo("独在异乡", "九月九日忆山东兄弟");
    run_demo("少小离家", "回乡偶书");
    run_demo("黄河远上", "凉州词  ");
    run_demo("秦时明月", "出塞    ");
    run_demo("寒雨连江", "芙蓉楼送辛渐");
    run_chat("你好");
    run_chat("你叫什么");
    run_chat("你会什么");
    run_chat("再见");
