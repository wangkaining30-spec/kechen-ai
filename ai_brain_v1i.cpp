// ============================================================
// ai_brain_v1i.cpp —— 手写数字识别神经网络（MNIST 风格，8x8 简化版）+ 交互识别
//
// 针对 Termux 环境的考虑：
...
//
// 网络结构：
//   输入层(64 像素) -> 隐藏层(256, ReLU) -> 输出层(10, Softmax)
//
// 训练：
...
//
// 编译（Termux）：
//   g++ -O2 -std=c++17 -o ai_brain_v1i ai_brain_v1i.cpp
// 运行：
//   ./ai_brain_v1i
//
// 交互识别：
//   训练完成后，程序把训练好的权重保存到 ai_brain_v1i_weights.bin，
//   再重新加载（模拟"加载训练好的权重"），自动演示一次识别
//   （0-9 干净数字），然后提示用户在终端输入自己的 8x8 点阵图案：
//   8 行，每行 8 个字符，'#'=笔画，'.'=空白；输入 q 或 quit 退出。
// ============================================================
...
public:
    ...
    // 将训练好的权重保存到二进制文件（先写网络结构，再写参数）
    bool saveWeights(const std::string& path) const {
        std::ofstream f(path, std::ios::binary);
        if (!f) return false;
        f.write((const char*)&nIn, sizeof(int));
        f.write((const char*)&nHid, sizeof(int));
        f.write((const char*)&nOut, sizeof(int));
        f.write((const char*)W1.data(), (long)W1.size() * sizeof(float));
        f.write((const char*)b1.data(), (long)b1.size() * sizeof(float));
        f.write((const char*)W2.data(), (long)W2.size() * sizeof(float));
        f.write((const char*)b2.data(), (long)b2.size() * sizeof(float));
        return f.good();
    }

    // 从二进制文件加载权重；文件不存在或网络结构不匹配时返回 false
    bool loadWeights(const std::string& path) {
        std::ifstream f(path, std::ios::binary);
        if (!f) return false;
        int ni = 0, nh = 0, no = 0;
        f.read((char*)&ni, sizeof(int));
        f.read((char*)&nh, sizeof(int));
        f.read((char*)&no, sizeof(int));
        if (!f || ni != nIn || nh != nHid || no != nOut) return false;
        f.read((char*)W1.data(), (long)W1.size() * sizeof(float));
        f.read((char*)b1.data(), (long)b1.size() * sizeof(float));
        f.read((char*)W2.data(), (long)W2.size() * sizeof(float));
        f.read((char*)b2.data(), (long)b2.size() * sizeof(float));
        return f.good();
    }
};
...
// 用网络识别一个 64 维输入，打印预测数字与置信度（可选打印完整概率分布）
void recognizeAndShow(const MLP& net, const std::vector<float>& x,
                      const std::string& tag, bool showDist) {
    std::vector<float> hp(net.nHid), h(net.nHid);
    std::vector<float> p = net.forward(x, hp, h);
    int pred = (int)(std::max_element(p.begin(), p.end()) - p.begin());
    std::cout << "  " << tag << " -> 预测数字: " << pred
              << "   置信度: " << std::fixed << std::setprecision(4) << p[pred] << "\n";
    if (showDist) {
        std::cout << "  概率分布:";
        for (int k = 0; k < 10; ++k)
            std::cout << "  " << k << "=" << std::fixed << std::setprecision(3) << p[k];
        std::cout << "\n";
    }
    printGlyph(x);
}
...
int main() {
    ...
    // ---------------- 保存并加载训练好的权重 ----------------
    const std::string wpath = "ai_brain_v1i_weights.bin";
    std::cout << "\n===== 权重保存 / 加载 =====\n";
    if (!net.saveWeights(wpath)) {
        std::cout << "警告: 无法保存权重到 " << wpath << "，将直接使用内存中的网络。\n";
    } else {
        std::cout << "训练好的权重已保存到: " << wpath << "\n";
    }

    MLP loadedNet(64, 256, 10, rng);   // 新建网络，模拟"从文件加载训练好的权重"
    if (loadedNet.loadWeights(wpath)) {
        std::cout << "已从 " << wpath << " 加载训练好的权重（结构 "
                  << loadedNet.nIn << "-" << loadedNet.nHid << "-" << loadedNet.nOut << "）。\n";
    } else {
        std::cout << "警告: 权重加载失败，将使用内存中的网络进行识别。\n";
        loadedNet = net;
    }
    ...
    for (const Glyph& g : kGlyphs) {
        ...
        for (int r = 0; r < 8; ++r)
            for (int c = 0; c < 8; ++c)
                if (pix(g, r, c)) x[r * 8 + c] = 1.0f;
        recognizeAndShow(loadedNet, x, "数字 " + std::to_string(g.label), false);
    }

    // ---------------- 交互识别：用户输入自己的 8x8 图案 ----------------
    std::cout << "\n===== 交互识别 =====\n";
    std::cout << "请输入你自己的 8x8 点阵图案：\n"
              << "  * 共 8 行，每行恰好 8 个字符；'#' = 笔画，'.' = 空白\n"
              << "  * 输入 q 或 quit 随时退出\n";
    ...
    while (true) {
        ...
        bool quit = false;
        std::cout << "\n> 输入图案（8 行）:\n";
        for (int r = 0; r < 8 && !quit; ++r) {
            while (true) {
                std::cout << "  第 " << (r + 1) << " 行: " << std::flush;
                std::string line;
                if (!std::getline(std::cin, line)) { quit = true; break; }  // 输入结束(EOF)
                size_t b = line.find_first_not_of(" \t\r");
                if (b == std::string::npos) {   // 空行
                    std::cout << "    (空行，请重新输入)\n";
                    continue;
                }
                size_t e = line.find_last_not_of(" \t\r");
                line = line.substr(b, e - b + 1);
                if (line == "q" || line == "quit") { quit = true; break; }
                if (line.size() != 8 || line.find_first_not_of("#.") != std::string::npos) {
                    std::cout << "    (无效: 需要恰好 8 个字符，且只能包含 # 和 .)\n";
                    continue;
                }
                rows.push_back(line);
                break;
        ...
        for (int r = 0; r < 8; ++r)
            for (int c = 0; c < 8; ++c)
                if (rows[r][c] == '#') x[r * 8 + c] = 1.0f;

        std::cout << "\n你输入的图案:\n";
        recognizeAndShow(loadedNet, x, "识别结果", true);
    }

    std::cout << "\n退出交互识别。训练完成。文件: ~/ai_brain_v1i.cpp\n";
    return 0;
}
