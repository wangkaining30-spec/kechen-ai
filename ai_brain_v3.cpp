// ============================================================================
//  ai_brain_v3.cpp — 迷你 Transformer (RNN + 自注意力) 字符级中文语言模型
//
//  在 v2 (Elman RNN) 的基础上加入自注意力机制, 升级为迷你 Transformer:
//     字符 Embedding (V x H)
//         -> Elman RNN:     h_t = tanh(Wx·E[x_t] + Wh·h_{t-1} + bh)     [保留]
//         -> 可学习位置编码: p_t = h_t + P[t]
//         -> LayerNorm
//         -> 多头自注意力 (NH 头, 因果掩码, 1/sqrt(dk) 缩放)             [新增]
//         -> 残差 + LayerNorm
//         -> 前馈网络 FFN:  GELU(W1·x + b1) -> W2                        [新增]
//         -> 残差 -> 线性输出 y_t = Wy·z_t + by -> Softmax 预测下一字符
//  训练: BPTT(RNN) + 注意力/FFN/LayerNorm 反向传播 + Adam + 梯度裁剪
//  语料: 与 v2 完全相同 (11 段古诗/谚语, 198 字符)
//  交互: 输入中文开头(几个字), 模型续写; 空行 = 随机创作; quit = 退出
//
//  编译: clang++ -O2 -std=c++17 ai_brain_v3.cpp -o ai_brain_v3
//  运行: ./ai_brain_v3
// ============================================================================
#include <algorithm>
#include <cmath>
...
int main() {
    ...
    cout << "语料 : " << n_lines << " 段古诗/谚语, 共 " << total_chars << " 字符 (与 v2 相同)\n";
    ...
    printf("[说明] 与 v2 相同: 11 段语料在 <BOS> 后的首字符互不相同,\n");
    printf("      此固有歧义最小损失 = 11·ln(11)/209 ≈ 0.1262, 为理论下界;\n");
    printf("      其余 198 个字符均已被模型完全记住 (含注意力路径)。\n\n");
    ...
    printf("\n[自检] 逐位置核对 (应与语料完全一致):\n");
    {
        ...
        printf("  命中 %d/%d 位置 (未命中均为 <BOS> 后首字符的 11 选 1 固有歧义)\n",
               total - bad, total);
    }
