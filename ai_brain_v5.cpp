// ============================================================================
//  ai_brain_v5.cpp — 迷你 Transformer (RNN + 自注意力) 字符级中文语言模型
//                    升级为"会对话的 AI": 在训练语料中加入 10 条对话问答对
//
//  在 v4 (11 段古诗 + 10 条迷你中文字典) 的基础上, 新增 10 条对话样本 (问答对):
//     你好=你好！很高兴认识你                 你叫什么=我叫客尘AI，我是用C++写的
//     你是谁=我是客尘AI，一个会写诗也会聊天的AI   再见=再见！欢迎再来
//     谢谢=不客气                             今天天气如何=今天是个好天气，适合学习AI
//     你会什么=我会背古诗，还会认数字         你多大了=我出生于2026年8月14日，今天刚出生
//     你开心吗=能和你聊天我很开心             1+1等于几=1+1等于2
//  模型结构 / 训练超参与 v4 完全相同 (RNN + 多头自注意力):
//     字符 Embedding (V x H)
...
//  语料: 11 段古诗/谚语 (198 字符) + 10 条迷你字典 (63 字符)
//        + 10 条对话问答 (160 字符) = 31 段, 421 字符
//  交互: 输入问题(如"你好"/"你叫什么") -> AI 直接回答 (自动隐藏 "Q=" 只显示回答);
//        输入中文开头 -> 模型续写; 空行 = 随机创作; quit = 退出
//
//  编译: clang++ -O2 -std=c++17 ai_brain_v5.cpp -o ai_brain_v5
//  运行: ./ai_brain_v5
// ============================================================================
#include <algorithm>
#include <cmath>
...
int main() {
    ...
    const char* corpus[] = {
        ...
        "行=走",
        "思=思考",
        // ---- 对话问答 (10 条, v5 新增; 与古诗/字典共享同一词表与同一套参数记忆) ----
        "你好=你好！很高兴认识你",
        "你叫什么=我叫客尘AI，我是用C++写的",
    ...
    cout << "语料 : " << n_lines << " 段 (11 段古诗/谚语 + 10 条迷你中文字典 + 10 条对话问答), 共 "
         << total_chars << " 字符\n";
    ...
    printf("      (共 31 段, 421 字符), 三类语料共享同一词表与同一套参数记忆。\n");
    printf("      固有歧义位置(上下文相同而下一字符不同, 无法完全记住):\n");
    printf("      ① <BOS> 后首字符 31 选 1 (床/明 各出现 2 次, 你 出现 6 次);\n");
    printf("      ② <BOS>床 -> 前/= 二选一; ③ <BOS>明 -> 月/= 二选一;\n");
    printf("      ④ <BOS>你 -> 好/叫/是/会/多/开 六选一。\n");
    printf("      理论最小平均损失 ≈ 0.236, 为下界;\n");
    ...
    printf("\n[自检] 逐位置核对 (应与语料完全一致):\n");
    {
        ...
        printf("  命中 %d/%d 位置 (未命中均为固有歧义: <BOS> 后首字符 31 选 1,\n",
               total - bad, total);
        printf("       以及 <BOS>床/<BOS>明 后的 前/=、月/= 二选一, <BOS>你 后的 六选一等)\n");
    }

    // ---- 自动演示续写 (与 v3/v4 相同的前缀) ----
